...
import car_drive_app.track.creator.points_and_circles
import car_drive_app.track.creator.points_and_splines
...
def creator_1(args: list=sys.argv) -> None:
    ...
    cr = car_drive_app.track.creator.points_and_circles.Creator(dimensions, track_save_name)
    cr.run()
...
def creator_2(args: list=sys.argv) -> None:
    ...
    dimensions = Vector(int(args[1]), int(args[2]))
    track_save_name = args[3]
    cr = car_drive_app.track.creator.points_and_splines.Creator(dimensions, track_save_name)
    cr.run()