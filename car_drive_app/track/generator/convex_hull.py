...
def next_most_left(start: Vector, others: list[Vector]) -> Vector:
    """Return the point in others that is the most right of the start Vector (and hence all others
    must be to the left).
    ...
    positive switches the pointer to current_point then continues.
    """
...
def convex_hull(points: list[Vector]) -> list[Vector]:
    ...
    Returns the points in clockwise order (anti-clockwise on the regular cartesian plane).
    """
